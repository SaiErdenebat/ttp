...
def barChart(tempDf, x, y):
    ...
    if 'symbol' in tempDf.columns and y == 'profitAndLoss' and x == 'indexNum':
        tempDf['text_label'] = tempDf['symbol'] + ': $' + tempDf[y].astype(str)
        fig = px.bar(tempDf, x=x, y=y, color='positive', color_discrete_map={True: 'green', False:'red'}, text='text_label')
    else:
